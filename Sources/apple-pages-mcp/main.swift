...
// Launcher only. Everything testable lives in PagesMCPCore, which the test target
...
do {
    ...
    // user_config into this process's arguments.
    try await PagesMCPServer.run(
    ...
    FileHandle.standardError.write(Data("apple-pages-mcp: \(error.localizedDescription)\n".utf8))
    exit(1)
}
